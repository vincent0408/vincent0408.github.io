...
document.getElementById('submit').onclick = function() {
    if (document.getElementById('answer').value == '109') {
        firebase.database().ref('question').update({ q5: 1 })
        document.getElementById('q6').classList.add('active')
        document.getElementById('q6').classList.remove('disabled')
        document.getElementById('q5').classList.remove('active')

        var q1icon = document.getElementById('q5').childNodes[1]['firstElementChild'].classList
        q1icon.remove('bi-x')
        q1icon.add('bi-check')
