...
function DrawSixCharts() {
    ...
    chart.database().ref().on('value', async snapshot => {
            ...
            var hw_interval = await getLabel('HW' + droplist.textContent.trim().slice(-1) + '-1')
            ...
            var submit = snap[user]['HW' + droplist.textContent.trim().slice(-1) + '-1' + ' Submit count']

            let col = []
            for (let i = 0; i < hw.length; i++) {
                col.push('gold')
            }
            console.log(submit, labels, getBarNo(labels, submit))

            col[getBarNo(labels, submit)] = 'orange'
            console.log(col)
            if (chart1) {
                chart1.data.datasets[0].data = hw
                chart1.data.labels = labels
                chart1.data.datasets[0].backgroundColor = col

                chart1.update();
            } else {
                chart1 = createChart(ctx1, labels, hw, '', col)
            }
        }
